...
//void buildCube(Mesh *mesh, float size);
...
void Draw() {
  ...
  float light_pos[3];
  light_pos[0]= 4.0 *cos(time*0.5);
  light_pos[1]= 3.0;
  light_pos[2]= 4.0 *sin(time*0.5);

  float eye_pos[3];
  eye_pos[0]= CAMLight->GetPosition().x;
  eye_pos[1]= CAMLight->GetPosition().y;
  eye_pos[2]= CAMLight->GetPosition().z;

  float far = cam->GetFar();
  /****TRANSFORMATIONS****/
  DRAWCubeLight->Translate(vec3(light_pos[0],light_pos[1],light_pos[2]));
  ...
  CAMLight->type = TPC_NORMAL;
  CAMLight->my_tex = CAMLight->my_tex;
  CAMLight->SetPosition((vec3(6.0,6.0,6.0)));//vec3(6.0*cos(time*0.5),6.0,6.0*sin(time*0.5)));//
...
/****************GEOMETRY************************/
	...
	geo_1->createPlane(20);
	//geo_1->createPlane(-5.0f,-2.5f,-5.0f,20.0f,20.0f);
	//geo_->createCube(4.0f,0.0f,6.0f,2.0f);
	geo_->createCube(2);
	geo_2->createPlane(-7.5f,0.0f,-5.0f,15.0f,15.0f);
	//cube_light01->createCube(-1.0f,0.0f,1.0f,2.0f);
	cube_light01->createCube(1);
	//geo_.createGeometry("../assets/cube.obj");
    //geo_->createGeometry(NULL,sizeof(array_),sizeof(relationalarray_),array_,relationalarray_);
...
/****************TEXTURES***********************/
	...
	//tex->CreateRenderDepthToTexture();
	//tex->Init("../assets/test.png");
	//texture_rock->Init("../assets/test1.jpg");
...
/****************UNIFORMS***********************/
	...
	//uniform_block1->InitUniform(mat_.get(),tex.get());
	...
	//uniform_block2->Set("u_color",color_2);
...
/****************DRAWABLES***********************/
	...
	/*********TARNSFORMATIONS*******************/
	//DRAWBaseCube->Translate(glm::vec3(13.0,1.0,1.0)); 
	DRAWBaseCube->Scale(vec3(1.5f,2.5f,1.5f));
	DRAWPlaneForRenderToTex->Scale(vec3(0.5f,0.5f,0.5f));
	DRAWCubeLight->Scale(vec3(0.5f,0.5f,0.5f));
	//DRAWBasePlane->Scale(vec3(0.5f,0.5f,0.5f));
...
/*********************************************/
	//skybox.alloc();
	//skybox->Init();
	//skybox->loadSkybox("../assets/","jajlands1_ft.jpg", "jajlands1_bk.jpg", "jajlands1_lf.jpg", "jajlands1_rt.jpg", "jajlands1_up.jpg", "jajlands1_dn.jpg");
