...
void Draw() {
  ...
  CAMShadows->ChangeLookAt(vec3(0,0,0),vec3(1.0*cos(time*0.5),1.0*cos(time*0.5), 1.0*sin(time*0.5)), vec3(0,1,0));
...
public:
	...
	int rol()
	{
		cout<<"**Correcto funcionamiento llamada a funciones ref_ptr"<<endl;
		return 1;
	}
private:
	int pepe;
...
/****************CAMERA***********************/
	...
	CAMShadows->Init(&CAMShadows->ChangePerspective(60.0f,1.0f, 25.0f, 0.01f),&CAMShadows->ChangeLookAt(vec3(0, 0, 0), vec3(0,0,3), vec3(0,1,0)),TPC_NORMAL);
